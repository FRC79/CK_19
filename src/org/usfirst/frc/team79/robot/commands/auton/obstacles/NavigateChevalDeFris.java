...
public class NavigateChevalDeFris extends CommandGroup {
	...
	public NavigateChevalDeFris() {
		...
		addSequential(new DriveDistanceEncoder(2)); // distance of robot to ramp
		addSequential(new SetPlow(Value.kForward));
		addSequential(new WaitForTime(0.5));
		addSequential(new DriveDistanceEncoder(2)); // not sure if I want to handle it that way, but drive through the obstacle
	}
